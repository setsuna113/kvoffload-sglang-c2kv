...
def test_internal_interval_uses_real_scheduler_first_token_gate():
    path = HTTP_SERVER.parents[1] / "managers/scheduler_output_processor_mixin.py"
    tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
    gate = next(
        node.value for node in ast.walk(tree)
        if isinstance(node, ast.Assign)
        and any(isinstance(target, ast.Name) and target.id == "should_output"
                for target in node.targets)
        and isinstance(node.value, ast.IfExp)
        and "stream_interval" in ast.unparse(node.value)
    )
    expression = compile(ast.Expression(body=gate), str(path), "eval")
    max_new_tokens = 32
    emitted = [
        length for length in range(1, max_new_tokens + 1)
        if eval(expression, {
            "req": SimpleNamespace(output_ids=[0] * length),
            "stream_interval": max_new_tokens + 1,
        })
    ]
    assert emitted == [1]
...
def _namespace(manager, plan, enabled, shadow_layer=None):
    return _load(
        ...
        get_bool_env_var=lambda name: enabled and name == "C2KV_NATIVE_BACKGROUND_EXTRAS",
...
async def test_selected_admitted_before_extras_and_response_waits_for_both():
    ...
    original_sampling = dict(native_request.sampling_params)
    task = asyncio.create_task(
        namespace["v1_c2kv_native_generate"](native_request, SimpleNamespace(headers={}))
    )
    ...
    assert manager.generation_input.sampling_params == {
        **original_sampling,
        "temperature": 0.0,
        "stream_interval": original_sampling["max_new_tokens"] + 1,
    }
    assert native_request.sampling_params == original_sampling
...
async def test_background_and_serial_preserve_output_cost_and_request_ids():
    ...
    assert {
        key: value
        for key, value in background_request.sampling_params.items()
        if key != "stream_interval"
    } == serial_request.sampling_params
    assert background_request.sampling_params["stream_interval"] == (
        background_request.sampling_params["max_new_tokens"] + 1
    )
...
async def test_cancel_aborts_generation_and_cancels_owned_extras():
    ...
    task = asyncio.create_task(
        helper["_c2kv_native_generate_with_background_extras"](
            Manager(), SimpleNamespace(rid="native-1", stream=False,
                                       sampling_params={"max_new_tokens": 2}), None,
            run_extras,
        )
    )
    await asyncio.wait_for(extra_started.wait(), 1)
