...
def test_generation_output_validation_accepts_finite_logprobs():
    output_ids, logprobs = native.validate_native_generation_output(
        [7, 8], [[-0.25, 7], [-1.5, 8]]
    )

    assert output_ids == [7, 8]
    assert logprobs == [-0.25, -1.5]


@pytest.mark.parametrize("value", [None, float("nan"), float("inf"), -float("inf")])
def test_generation_output_validation_rejects_nonfinite_sampler_output(value):
    row = value if value is None else [value, 0]
    with pytest.raises(
        native.NativeGenerationOutputError,
        match=native.NONFINITE_LOGITS_ERROR_CODE,
    ) as captured:
        native.validate_native_generation_output([0], [row])

    payload = native.generation_output_error_payload(
        error=captured.value,
        rid="rid-1",
        session_id="session-1",
        generation_id="generation-1",
    )
    assert payload["error"]["code"] == native.NONFINITE_LOGITS_ERROR_CODE
    assert payload["generation"] == {
        "schema": native.NATIVE_GENERATION_FAILURE_SCHEMA,
        "status": "failed",
        "rid": "rid-1",
        "session_id": "session-1",
        "generation_id": "generation-1",
...
def test_v7_null_logprob_shape_fails_closed_without_rewriting_tokens():
    output_ids = [0] * 4096
    raw_logprobs = [None] * 4096

    with pytest.raises(native.NativeGenerationOutputError) as captured:
        native.validate_native_generation_output(output_ids, raw_logprobs)

    assert captured.value.code == native.NONFINITE_LOGITS_ERROR_CODE
    assert output_ids == [0] * 4096


def test_extraction_budget_failure_reports_completed_work_before_denied_miss():
    payload = native.extraction_budget_error_payload(
        message=(
            "C2KV_EXTRACTION_BUDGET_EXHAUSTED: the exact chunk is not cached "
            "and this request may not schedule another encoder pass"
        ),
        requested_chunks=336,
        unique_chunks=336,
        cache_hits=44,
        cache_misses=172,
        max_extraction_calls=172,
    )

    assert payload["error"]["code"] == native.EXTRACTION_BUDGET_ERROR_CODE
    assert payload["extraction"] == {
        "schema": native.NATIVE_EXTRACTION_FAILURE_SCHEMA,
        "requested_chunks": 336,
        "unique_chunks": 336,
        "processed_chunks": 216,
        "cache_hits": 44,
        "cache_misses": 172,
        "model_calls": 172,
        "max_extraction_calls": 172,
        "failed_chunk_index": 216,
    }
...
def test_extraction_budget_failure_rejects_inconsistent_partial_counts(change):
    arguments = {
    ...
    with pytest.raises(ValueError):
        native.extraction_budget_error_payload(**arguments)
