...
def _mixin_methods():
    ...
    methods = [
        ...
        and node.name
        in {
            "init_communicators", "c2kv_extract", "c2kv_background_extract_many",
            "c2kv_bulk_cache_lookup",
        }
    ]
    ...
    namespace = {
        ...
        "C2KVExtractBatchReqOutput": object,
...
@pytest.mark.asyncio
async def test_background_group_uses_flagged_envelope_and_returns_retry():
    method = _mixin_methods()["c2kv_background_extract_many"]
    sent = []
    ...
    manager = SimpleNamespace(
        auto_create_handle_loop=lambda: None,
        c2kv_extract_batch_communicator=communicator,
        c2kv_extract_batch_collector=SimpleNamespace(
            submit=lambda _req: pytest.fail("Background group entered foreground collector")
        ),
    )
    items = [
        {
            "rid": f"prewarm:owner:job:{index}", "input_ids": [index + 1],
            "input_text": "", "compression_ratio": 8, "allow_cache_miss": True,
            "outer_request_id": "outer", "measurement_phase": "phase",
            "projection_set": "history", "background_extraction": True,
        }
        for index in range(2)
    ]
    assert await method(manager, items) is retry
    assert len(sent) == 1 and sent[0].background_extraction is True
    assert isinstance(sent[0].rid, str) and sent[0].rid
    assert sent[0].c2kv_outer_request_id == "outer"
    assert sent[0].c2kv_measurement_phase == "c2kv_cross_turn_prewarm:extraction_batch"
    assert [item.rid for item in sent[0].items] == [item["rid"] for item in items]
    assert all(item.background_extraction for item in sent[0].items)
    assert all(item.c2kv_outer_request_id == "outer" for item in sent[0].items)
    assert all(item.c2kv_measurement_phase == "phase" for item in sent[0].items)
    assert all(item.allow_cache_miss for item in sent[0].items)


@pytest.mark.asyncio
async def test_background_group_rejects_wrong_dp_count_and_output_order():
    method = _mixin_methods()["c2kv_background_extract_many"]
    items = [
        {
            "rid": rid, "input_ids": [1], "input_text": "",
            "compression_ratio": 8, "allow_cache_miss": True,
            "outer_request_id": None, "measurement_phase": None,
    ...
    with pytest.raises(RuntimeError, match="one DP reply"):
        await method(manager, items)
    replies.append(SimpleNamespace(
        success=True, retry_individually=False,
        items=[SimpleNamespace(rid="second"), SimpleNamespace(rid="first")],
    ))
