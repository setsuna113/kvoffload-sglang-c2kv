...
def chat_template_tools_dump(tools) -> List[Dict[str, Any]]:
    """Serialize `Tool` models for the chat template exactly as the client sent
    them (`exclude_unset`), so the tool prologue the server renders is
    token-identical to what a client that tokenizes the same OpenAI tool JSON
    itself would get. `model_dump()` without it adds pydantic defaults such as
    `"strict": false` (+4 tokens per tool with the Qwen3 template), which shifts
    every C2KV insertion point and repair position by a constant the client
    cannot see. See c2kv/c2kv_serving_semantics.md."""
...
class C2KVRepairExtractResponse(BaseModel):
    ...
    # True when K was captured post-RoPE (serving_cache source) and can only be
    # placed at its original positions; False = pre-RoPE, re-rotatable.
