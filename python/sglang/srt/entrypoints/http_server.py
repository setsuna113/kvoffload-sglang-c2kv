...
def _c2kv_native_capability() -> Dict[str, Any]:
    ...
    return {
        ...
        "exact_state": {
            ...
            "enabled": os.environ.get("C2KV_ENABLE_EXACT_STATE") == "1",
            "endpoint": "/v1/c2kv/exact_state",
            "boundary": "completed_request_quiescent",
        },
...
@app.post("/v1/c2kv/exact_state", response_class=SGLangORJSONResponse)
async def v1_c2kv_exact_state(raw_request: Request):
    """Snapshot an explicitly isolated engine; never replay a prompt."""
    if os.environ.get("C2KV_ENABLE_EXACT_STATE") != "1":
        return SGLangORJSONResponse({"error": "Exact state endpoint is disabled"}, status_code=403)
    payload = await raw_request.json()
    if not isinstance(payload, dict) or payload.get("operation") not in {"capture", "restore", "release"}:
        return SGLangORJSONResponse({"error": "Invalid exact state operation"}, status_code=400)
    result = await _global_state.tokenizer_manager.c2kv_exact_state(
        payload["operation"], payload.get("snapshot_id")
    )
    if not result.success:
        return SGLangORJSONResponse({"error": result.error}, status_code=409)
    return SGLangORJSONResponse(result.state)
...
async def v1_c2kv_native_generate(
    ...
    try:
        ...
        for index, chunk in enumerate(plan.unique_chunks):
            ...
            if not result.success:
                if isinstance(result.error, str) and result.error.startswith(
                    ...
                    payload = extraction_budget_error_payload(
                    ...
                    logger.error("[c2kv-native] request rejected: %s", result.error)
                    return ORJSONResponse(
                        payload, status_code=HTTPStatus.BAD_REQUEST
                    )
        ...
        raw_output_ids = list(generated.get("output_ids") or [])
        raw_logprobs = list(meta_info.get("output_token_logprobs") or [])
        try:
            output_ids, token_logprobs = validate_native_generation_output(
                raw_output_ids, raw_logprobs
            )
        except NativeGenerationOutputError as error:
            logger.error("[c2kv-native] generation failed: %s", error)
            return ORJSONResponse(
                generation_output_error_payload(
                    error=error,
                    rid=base_rid,
        ...
        if str(error).startswith(NONFINITE_LOGITS_ERROR_CODE + ":"):
            native_error = NativeGenerationOutputError(
                NONFINITE_LOGITS_ERROR_CODE,
